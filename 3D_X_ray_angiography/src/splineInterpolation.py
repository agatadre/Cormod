...
def cubicSplineInterpolate(x_axis, y_axis, z_axis, numberOfInterpolation=100):
    '''
    ...
    # plotCubicSpline(U, V, W, x_axis, y_axis, z_axis, numberOfInterpolation)
    return get_cubic_spline_points(U, V, W, x_axis, y_axis, z_axis, numberOfInterpolation)
...
def plotCubicSpline(U, V, W, x_axis, y_axis, z_axis, numberOfInterpolation):
    m = 1
    xLinespace = []
    yLinespace = []
    zLinespace = []
...
def get_cubic_spline_points(U, V, W, x_axis, y_axis, z_axis, numberOfInterpolation):
    m = 1
    xLinespace = []
    yLinespace = []
    zLinespace = []
...
# y_axis = [2, 3]
# z_axis = [3, 4]
# #spline = cubicSplineInterpolate(x_axis, y_axis, z_axis)
# coords = [x_axis, y_axis, z_axis]
# tck, u = interpolate.splprep(coords, w=np.ones(len(coords[0])), k=1, s=5000)
# num_true_pts = 200
# u_fine = np.linspace(0, 1, num_true_pts)
# spline = interpolate.splev(u_fine, tck)
#
# # visualization
# fig = plt.figure()
# my_ax = fig.add_subplot(111, projection='3d')
